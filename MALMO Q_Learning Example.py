...
class TabQAgent(object):
    ...
    def act(self, world_state, agent_host, current_r):
        ...
        #TODO - Need to amend this for Z-Values as well - for height?
        ...
        if self.training and self.prev_s is not None and self.prev_a is not None:
            ...
            self.q_table[self.prev_s][self.prev_a] = old_q + \
                        self.alpha * (current_r + self.gamma * max(self.q_table[current_s]) - old_q)
        ...
        if random.random() < self.epsilon:
            ...
            a = l[y]
            #TODO - Can't we just use m??
            self.logger.info("Taking q action: %s" % self.actions[a])

        # send the selected action
        agent_host.sendCommand(self.actions[a])
        #TODO - Change this to send action in MARLO
